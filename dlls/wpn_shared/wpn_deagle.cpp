...
void CDEAGLE::PrimaryAttack()
{
	...
	{
		DEAGLEFire(1.0 * (1 - m_flAccuracy), 0.2, FALSE);
	}
	else if (m_pPlayer->pev->flags & FL_DUCKING)
	{
		DEAGLEFire(0.0375 * (1 - m_flAccuracy), 0.2, FALSE);
	}
	else if (m_pPlayer->pev->velocity.Length2D() > 0)
	{
		DEAGLEFire(0.5 * (1 - m_flAccuracy), 0.2, FALSE);
	}
	else
	{
		DEAGLEFire(0.05 * (1 - m_flAccuracy), 0.2, FALSE);
	}
}
...
void CDEAGLE::DEAGLEFire(float flSpread, float flCycleTime, BOOL fUseSemi)
{
	...
	flCycleTime -= 0.075f;flCycleTime -= 0.05f;
